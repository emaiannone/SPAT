...
//		 } //This is the old single thread.
    ...
    public static void main(String[] args) throws IOException {
        Utils.maxTrans = 10000;
        if (args.length < 5) {
            System.out.println("SPAT needs at least five arguments to run properly: "
                    + "[seedforRandom] [theIdOfTheSelectedRule] [DirPathOftheSourcefiles] [OutputDir] [PathoftheJre(rt.jar)] & [PathofotherDependentJar]"
                    + "for example \"C:\\Program Files\\Java\\jre1.8.0_211\\lib\\rt.jar\"");
            System.exit(4);
        }
        long seed = Long.parseLong(args[0]);
        Utils.setSeedRandom(seed);
        String ruleID = args[1];
        String rootDirToNaturalProg = args[2];
